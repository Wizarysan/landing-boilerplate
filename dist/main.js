...
(function(factory) {
    if (true) {
        !(__WEBPACK_AMD_DEFINE_ARRAY__ = [ __webpack_require__(10), __webpack_require__(1), __webpack_require__(2) ], __WEBPACK_AMD_DEFINE_FACTORY__ = (factory),
...
function () {
  ...
  form_createClass(Form, [{
    ...
    value: function check() {
      ...
      this.inputs.forEach(function (el) {
        el.classList.remove(_this3.inputDom.fieldWarningClass);

        if (_this3.checkModel[el.name]) {
          if (el.value.search(_this3.checkModel[el.name]) < 0) {
            _this3.printError(el);
    ...
    value: function send() {
      if (this.url) {} else {
        ...
        fetch(this.form.dataset.send, params).then(function (response) {
...
var src_createQuiz = function createQuiz() {
  var testQuiz = new Quiz('#test-quiz', 't-quiz', 'Тестовый Квиз', [{
    ...
    answers: [{
      ...
      text: 'Научно-познавательные фильмы',
      image: 'q1-4.jpg'
    }]
  }, {
